...
def test_delete(cursor, users, products, orders, reviews, order_items):
    print("🗑️ DELETE...")
    u_count, p_count, o_count, r_count, oi_count = len(users) // 2, len(products) // 2, len(orders) // 2, len(reviews) // 2, len(order_items) // 2
    log_result("delete", "reviews", measure_time(lambda: [
        cursor.execute("DELETE FROM reviews WHERE id = %s", (rid,))
        for rid in sample_values(reviews, "id", r_count)
    ]), r_count)

    log_result("delete", "order_items", measure_time(lambda: [
...
def main():
    ...
    test_delete(cursor, users, products, orders, reviews, order_items)
    conn.commit()
